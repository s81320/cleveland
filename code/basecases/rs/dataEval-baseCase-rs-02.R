...
summary(lm2)
# this is so poor :-( ... when compared to the first model
# for 10 repetitions , nRS = 10
# R sqrd almost 0 , mean oob acc and mean d1 dissim not significant
# for nRS = 100
# all predictors are significant , the res std error is still high at 6% , R sqres goes up, now around 6%

# also: it is better than the base case with only the size information (in both nRS cases):
anova(lm2, lm(acc.ratio~size, data=rs), test='Chisq')
# anova(lm(acc.ratio~size+mean.oob.acc, data=rs), lm(acc.ratio~size, data=rs), test='Chisq')

# at nRS=10 nothing is indicating we need the dissimilarity measure ...
# at nRS=100 it is ...
